...
;(function($){ "use strict";
    ...
    Iconpicker.prototype.bindEvents = function () {
        ...
        op.table.find('.btn-icons').off('click').on('click', function(e) {
            e.preventDefault();
            el.select($(this).val());
            if(op.inline === false){
    ...
    Iconpicker.prototype.updateIcons = function (page) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            for (var j = 0; j < op.cols; j++) {
                ...
                var btn = $('<button class="btn ' + op.unselectedClass + ' btn-icons"></button>').hide();
                if (pos < op.icons.length) {
                    var v = op.iconClassFix + op.icons[pos];
                    btn.val(v).attr('title', v).append('<i class="' + op.iconClass + ' ' + v + '"></i>').show();
                    if (op.icon === v) {
                        btn.addClass(op.selectedClass).addClass('btn-icons-selected');
                    }
                }
    ...
    Iconpicker.prototype.setSelectedClass = function (value) {
        this.options.selectedClass = this.removeAddClass('.btn-icons-selected', this.options.selectedClass, value);
    };

    Iconpicker.prototype.setUnselectedClass = function (value) {
        this.options.unselectedClass = this.removeAddClass('.btn-icons', this.options.unselectedClass, value);
    };
