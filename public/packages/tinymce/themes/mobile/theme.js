...
(function () {
    ...
    var getToolbarIconButton = function (clazz, editor) {
      ...
      return optOxideIcon.fold(function () {
        return dom$1('<span class="${prefix}-toolbar-button ${prefix}-toolbar-group-item ${prefix}-icons-' + clazz + ' ${prefix}-icons"></span>');
      }, function (icon) {
    ...
    var makeItems = function (spec$1) {
      return [
        spec('<span class="${prefix}-toolbar-button ${prefix}-icons-small-font ${prefix}-icons"></span>'),
        makeSlider(spec$1),
        spec('<span class="${prefix}-toolbar-button ${prefix}-icons-large-font ${prefix}-icons"></span>')
      ];
    };
    ...
    var field = function (name, placeholder) {
      ...
      var buttonSpec = record(Button.sketch({
        dom: dom$1('<button class="${prefix}-input-container-x ${prefix}-icons-cancel-circle ${prefix}-icons"></button>'),
        action: function (button) {
    ...
    var sketch$3 = function (rawSpec) {
      ...
      var navigationButton = function (direction, directionName, enabled) {
        return Button.sketch({
          dom: dom$1('<span class="${prefix}-icons-' + directionName + ' ${prefix}-icons"></span>'),
          action: function (button) {
      ...
      var memDots = record({
        ...
        components: bind$3(spec$1.fields, function (_f, i) {
          return i <= spec$1.maxFieldIndex ? [spec('<div class="${prefix}-dot-item ${prefix}-icons-full-dot ${prefix}-icons"></div>')] : [];
        })
      });
      return {
            ...
            components: collapsable ? [
              {
                dom: {
                  tag: 'span',
                  classes: [resolve('styles-collapse-icons')]
                }
              },
              text(value)
            ] : [text(value)],
    ...
    var sketch = function (onView, _translate) {
      var memIcon = record(Container.sketch({
        dom: dom$1('<div aria-hidden="true" class="${prefix}-mask-tap-icons"></div>'),
        containerBehaviours: derive$2([Toggling.config({
            toggleClass: resolve('mask-tap-icons-selected'),
            toggleOnExecute: false
          })])
      }));
      ...
      return Container.sketch({
        ...
        components: [Container.sketch({
            ...
            components: [Button.sketch({
                ...
                buttonBehaviours: derive$2([Toggling.config({ toggleClass: resolve('mask-tap-icons-selected') })])
              })]
          })]
      });
    };
    ...
    var makeEditSwitch = function (webapp) {
      return build$1(Button.sketch({
        dom: dom$1('<div class="${prefix}-mask-edit-icons ${prefix}-icons"></div>'),
        action: function () {
