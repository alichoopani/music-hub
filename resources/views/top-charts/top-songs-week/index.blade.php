    ...
    <div class="max-w-[81%] mx-auto">
        <div class="py-2 mt-40 lg:my-0">
            <div class="flex flex-wrap">
                <div class="uppercase text-white text-2xl lg:text-3xl font-extrabold cursor-default">top songs week</div>
                ...
                @for ($count = 1; $count <= 20; $count++)
                    <div class="w-full grid gap-4 grid gap-4 grid-cols-1 grid-rows-1 my-4">
                        <div class="bg-[#303030] flex rounded-md">
                            <a class="w-60" href="{{route('trackDetail', ['id' => $topCharts[$count-1]->id ."-". $topCharts[$count-1]->title])}}">
                                <img class="cursor-pointer hover:opacity-60 h-6 min-h-full rounded-l-md aspect-square object-cover flex-shrink-0"
                                    src="{{asset('storage/' . $topCharts[$count-1]->image)}}" alt="music"></a>
                            <div class="mx-4 my-8 text-[#979797] text-xl cursor-default">{{$count}}</div><div
                                ...
                                <a href="{{route('trackDetail', ['id' => $topCharts[$count-1]->id ."-". $topCharts[$count-1]->title])}}">
                                    <span class="hover:text-[#909090] cursor-pointer paragraph-ellipsis-2 leading-snug w-40">
                                        {{$topCharts[$count-1]->title}}</span>
                                </a>
                                <div class=" capitalize text-sm text-[#989896] font-semibold w-40">
                                    <a href="{{route('artistDetail', ['id' => $topCharts[$count-1]->id ."-". $topCharts[$count-1]->title])}}">
                                        <span class="hover:underline cursor-pointer">{{$topCharts[$count-1]->artist->title}}</span>
                                    </a>
                                </div>
                            </div>
                            <div class="w-full text-[#686868] font-semibold my-8 flex text-center gap-2 flex-row-reverse">
                                ...
                                <div class="flex-1 cursor-default my-auto hidden lg:flex">{{ $topCharts[$count-1]->publish->format('d/m/Y') }}</div>
                                <div class="flex-1 cursor-default my-auto hidden lg:flex">{{ $topCharts[$count-1]->likes->count() }} likes</div>
                                <div class="flex-1 cursor-default my-auto hidden lg:flex">{{ $topCharts[$count-1]->plays }} plays</div>
                            </div>
                        </div>
                    </div>
                @endfor
